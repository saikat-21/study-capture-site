...
export async function sendExtensionActivation({ extensionId, email, activationGrant }) {
  const safeExtensionId = sanitizeExtensionId(extensionId);
  if (!safeExtensionId) {
    return {
      ok: false,
      skipped: true,
      message: "Extension ID missing"
    };
  }

  const bridgeResult = await sendPostMessageActivation({
    email,
    activationGrant,
  ...
  if (globalThis.browser?.runtime?.sendMessage) {
    try {
      ...
      console.error("Study Capture extension activation handoff failed.", error);
      return {
        ok: false,
        message: "Activation failed to connect to extension. Please reopen extension and try again."
      };
    }
  }
...
function sendPostMessageActivation({ email, activationGrant, extensionId }) {
...
function sendChromeRuntimeMessage(extensionId, message) {
  return new Promise((resolve) => {
    globalThis.chrome.runtime.sendMessage(extensionId, message, (response) => {
      ...
      if (lastError) {
        console.error("Study Capture extension activation sendMessage failed.", lastError);
        resolve({
          ok: false,
          message: "Activation failed to connect to extension. Please reopen extension and try again."
        });
