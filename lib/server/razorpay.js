...
export function resolveCheckoutPricing({ testMode, testToken } = {}) {
  const publicPriceInr = resolveCurrentPublicPriceInr();
...
function resolveCurrentPublicPriceInr() {
  const configuredPrice = readPositiveIntegerEnv(
    "PUBLIC_PRICE_INR",
    DEFAULT_PUBLIC_PRICE_INR
  );

  if (configuredPrice !== DEFAULT_PUBLIC_PRICE_INR) {
    console.warn("checkout_public_price_env_ignored", {
      configuredPublicPriceInr: configuredPrice,
      enforcedPublicPriceInr: DEFAULT_PUBLIC_PRICE_INR
    });
  }

  return DEFAULT_PUBLIC_PRICE_INR;
}
