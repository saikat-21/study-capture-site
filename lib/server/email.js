...
export async function sendEmail({ to, subject, html, text }) {
  ...
  const from = process.env.RESEND_FROM_EMAIL || "Study Capture <support@studycapture.co>";
...
export async function sendWelcomeEmail(email, licenseRef) {
  const message = buildWelcomeEmail({ email, licenseRef });
  try {
